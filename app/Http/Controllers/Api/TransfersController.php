...
class TransfersController extends RestController
{
    ...
    private function transferQuery(Request $request)
    {
        ...
        if ($request->ordertype == 'DESC') {
            $transfers = $my_transfers->merge($for_sale)
...
//get transfer for sale according to shift time
    ...
    public function history(Request $request)
    {
        ...
        foreach ($result as $item) {
            ...
            if (empty($item->driver->employer->profile_pic))
                $driver_pic = $default_img;
            else
                $driver_pic = url('uploads/drivers/' . $item->driver->employer->profile_pic);
            ...
            if ($item->type == 'arrival') {
                if (empty($item->host->profile_pic))
                    $host_image = $default_img;
                else
                    $host_image = url('uploads/hosts/' . $item->host->profile_pic);
                if ($item->host_id)
                    $array['host'] = [
                        'id' => $item->host->id,
                        'first_name' => $item->host->employer->first_name,
